...
{
	uint32 *Wroking_Set=kmalloc(numOfElements*sizeof(struct WorkingSetElement));
	return (void*)Wroking_Set;
}
...
void env_free(struct Env *e)
{
	...
	uint32 Max_Index_Page_Table=PDX(USER_TOP);
	i=0;
	for(;i<Max_Index_Page_Table;++i){
		uint32 Entery=e->env_page_directory[i];
		uint32 Frame_Num=(Entery >> 12);
		uint32 Pa_Page_Table=Frame_Num*PAGE_SIZE;
		uint32* Va_Page_Table =(uint32*)kheap_virtual_address((unsigned int)Pa_Page_Table);
		kfree((void*)Va_Page_Table);
	}
