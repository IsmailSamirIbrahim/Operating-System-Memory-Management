...
void Placement(struct Env * curenv, uint32 fault_va){
	...
	//[3]Check If the page does not exist on page file
	if(Ret == E_PAGE_NOT_EXIST_IN_PF){
		//[4]Check if it is a stack page
		if(fault_va > USTACKBOTTOM && fault_va < USTACKTOP){
			//[5]add a new empty page with this faulted address to page file
	...
	int k=0;
	for(;k<curenv->page_WS_max_size;++k){
		if(env_page_ws_is_entry_empty(curenv,k)){
			env_page_ws_set_entry(curenv,k,fault_va);
			break;
		}
	}

	if(k == (curenv->page_WS_max_size-1))
		curenv->page_last_WS_index  = 0;
	else{
		curenv->page_last_WS_index = k+1;
	}
...
void page_fault_handler(struct Env *curenv, uint32 fault_va)
{
	...
	if(env_page_ws_get_size(curenv) < curenv->page_WS_max_size){
		...
		if(isPageReplacmentAlgorithmCLOCK()){
			while(1==1){
				if((pt_get_page_permissions(curenv,curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address) & PERM_USED) !=PERM_USED){ //Used=0.
					Victim_Index=curenv->page_last_WS_index;
					Victim=curenv->ptr_pageWorkingSet[curenv->page_last_WS_index];
					break;
				}
				else{
					pt_set_page_permissions(curenv,curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,PERM_USED,PERM_USED);
			...
			int Try1=1,Try2=0;
			uint32 Fisrt_Position=curenv->page_last_WS_index;
			while(1==1){
				if(Try1){
					if(((pt_get_page_permissions(curenv,curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address) & PERM_USED) !=PERM_USED) //Used=0.
							&&((pt_get_page_permissions(curenv,curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address) & PERM_MODIFIED) !=PERM_MODIFIED)){
						Victim_Index=curenv->page_last_WS_index;
						Victim=curenv->ptr_pageWorkingSet[curenv->page_last_WS_index];
						break;
					}
					...
					if(curenv->page_last_WS_index == Fisrt_Position)
						Try1=0,Try2=1;
				}
				else if(Try2){
					if((pt_get_page_permissions(curenv,curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address) & PERM_USED) !=PERM_USED){ //Used=0.
			...
			for(;i<curenv->page_WS_max_size;++i){
				if(env_page_ws_get_time_stamp(curenv,i) < Min){
					...
					Victim_Index= i;
					Victim=curenv->ptr_pageWorkingSet[i];
				}
			}
		}
		...
		//check if victim_page not_modified.
		if((pt_get_page_permissions(curenv,Victim.virtual_address) & PERM_MODIFIED) != PERM_MODIFIED){
			unmap_frame(curenv->env_page_directory,(void*)Victim.virtual_address);
			curenv->nNotModifiedPages++;
		}
		//victim_page is modified.
		else{
