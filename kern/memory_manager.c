...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* Phys_Page_Table=(uint32*)kheap_physical_address((unsigned int)Ptr_Page_Table);

	ptr_page_directory[PDX(virtual_address)] = ((uint32)Phys_Page_Table |PERM_USER|PERM_PRESENT| PERM_WRITEABLE);
...
/*	//TODO: [PROJECT 2016 - Dynamic Deallocation] freeMem() [Kernel Side]
	...
	uint32 End_Va =(virtual_address+ROUNDUP(size,PAGE_SIZE));
	i=0;
	...
	for(;i<Num_Of_Pages;++i){
		Ptr_Page_Table=NULL;
		get_page_table(e->env_page_directory,(void*)Start_Va,&Ptr_Page_Table);
		if(Ptr_Page_Table !=NULL){
			...
			if(count == 1024){
				...
				pd_clear_page_dir_entry(e,(uint32)Start_Va);
			}
		}
		Start_Va+=PAGE_SIZE;
	}
...
void moveMem(struct Env* e, uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	...
	uint32 Start_Va=src_virtual_address;
	uint32 End_Va=(src_virtual_address +ROUNDUP(size,PAGE_SIZE));
	uint32 Dest=dst_virtual_address;

	struct Frame_Info *Ptr_Frame_Info=NULL;
	uint32* Ptr_Page_Table=NULL;

	for (;Start_Va < End_Va ;Start_Va +=PAGE_SIZE ){
		Ptr_Frame_Info=get_frame_info(e->env_page_directory,(void*)Start_Va,&Ptr_Page_Table);
		pf_update_env_page(e,(void*)Dest, Ptr_Frame_Info);
		Dest +=PAGE_SIZE;
	}

	freeMem(e,src_virtual_address,size);
